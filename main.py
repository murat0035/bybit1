...
# ✅ MEXC DEMO API URL
MEXC_API_URL = "https://api.mexc-demo.com"
...
# ✅ MEXC DEMO API Üzerinden Bakiye Sorgulama
...
def get_balance():
    try:
        ...
        logging.info(f"MEXC DEMO API Yanıtı: {data}")
        ...
        if "balances" in data:
            ...
            return jsonify({"error": "MEXC DEMO API beklenen formatta yanıt vermedi.", "response": data})
...
# ✅ BTC/USDT Piyasa Fiyatını Al (DEMO)
...
def get_price():
    try:
        ...
        logging.info(f"MEXC DEMO'dan Gelen Fiyat Verisi: {data}")
...
# ✅ Piyasa Fiyatından BTC Al (Market Order - DEMO)
...
def buy_order():
    try:
        ...
        logging.info(f"BTC Alım Yanıtı (DEMO): {data}")

        return jsonify(data)

    except Exception as e:
        logging.error(f"BTC satın alma hatası (DEMO): {e}")
        return jsonify({"error": str(e)})

# ✅ Piyasa Fiyatından BTC Sat (Market Order - DEMO)
...
def sell_order():
    try:
        ...
        logging.info(f"BTC Satış Yanıtı (DEMO): {data}")

        return jsonify(data)

    except Exception as e:
        logging.error(f"BTC satma hatası (DEMO): {e}")
        return jsonify({"error": str(e)})

# ✅ API'nin Çalıştığını Kontrol Etmek İçin Ana Sayfa
@app.route("/")
def home():
    return "✅ MEXC DEMO API bağlantısı aktif! BTC/USDT işlemleri için hazır."
